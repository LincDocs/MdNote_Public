...
如果您使用 process-unmapped-keys yes 或在 defsrc 中拥有密钥，Windows 有一个选项可以减轻 AltGr (ralt) 的奇怪行为。这适用于许多非美国布局。您可以使用列出的值之一来更改 kanata 使用该密钥执行的操作：
...
|   |   |
|---|---|
|笔记|即使使用这些解决方法，将 lctl 和 ralt 放入 defsrc 中可能无法与也使用键盘拦截的其他应用程序正常工作。已知存在问题的应用程序：GWSL/VcXsrv|
