---
title: "构建一个跨浏览器的扩展程序"
source: "https://developer.mozilla.org/zh-CN/docs/Mozilla/Add-ons/WebExtensions/Build_a_cross_browser_extension"
author:
published: 2025-08-05
created: 2026-05-02
description: "浏览器扩展 API 的引入为浏览器扩展的开发创造了“一次开发跨浏览器”的前景。然而，在使用扩展 API 的浏览器中 (主要是 Chrome、Firefox、Opera 和 Edge) ，API 的实现和覆盖范围都存在差异。除此之外，Safari 使用了它自己的 Safari 扩展脚本系统。"
tags:
  - "clippings"
---
# 构建一个跨浏览器的扩展程序

浏览器扩展 API 的引入为浏览器扩展的开发创造了“一次开发跨浏览器”的前景。然而，在使用扩展 API 的浏览器中 (主要是 *Chrome、Firefox、Opera 和 Edge*) ，API 的实现和覆盖范围都存在*差异*。除此之外，*Safari* 使用了它自己的 Safari 扩展脚本系统。

最大化兼容浏览器扩展意味着至少在两个不同的浏览器上兼容同一个扩展。本文探讨了在创建跨浏览器扩展时所面临的六个主要挑战，并在每种情况下提出了如何应对这些挑战。

本文*不讨论为 Safari 构建浏览器扩展*。你可以通过 Safari 扩展共享一些资源，比如图片和 HTML 内容。然而，如果你要进行 JavaScript 部分的编程则需要作为一个单独的开发项目进行，除非你希望创建自己的 polyfill。
...
**备注：** 如果你对这两种方法之间的差异不熟悉，可以看一下 [了解异步 JavaScript: Callbacks、Promises 和 Async/Await](https://medium.com/codebuddies/getting-to-know-asynchronous-javascript-callbacks-promises-and-async-await-17e0673281ee) 或者 MDN 的 [Using promises](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Guide/Using_promises) 页面。
...
- **缺乏对整个功能的支持。** 例如，在撰写本文时，Edge 没有提供对 [隐私](https://developer.mozilla.org/zh-CN/docs/Mozilla/Add-ons/WebExtensions/API/privacy#browser_compatibility) 功能的支持。
...
你可以在 [Mozilla Developer Network 浏览器对 JavaScript API 页面的支持](https://developer.mozilla.org/zh-CN/docs/Mozilla/Add-ons/WebExtensions/Browser_support_for_JavaScript_APIs) 上找到 4 个主要浏览器对扩展 API 的支持细节，以及 Firefox for Android 对扩展 API 的支持细节。浏览器兼容性信息也包含在每个函数及其方法、类型和事件的 Mozilla Developer Network [JavaScript APIs](https://developer.mozilla.org/zh-CN/docs/Mozilla/Add-ons/WebExtensions/API) 参考页面中。
...
主要浏览器支持的 [`manifest.json`](https://developer.mozilla.org/zh-CN/docs/Mozilla/Add-ons/WebExtensions/manifest.json) 文件字段的差异大致可分为三类：

- **扩展信息属性。** 例如，在撰写本文时，Firefox 和 Opera 包含和 [`author`](https://developer.mozilla.org/zh-CN/docs/Mozilla/Add-ons/WebExtensions/manifest.json/author#browser_compatibility) 地位相等的 [`developer`](https://developer.mozilla.org/zh-CN/docs/Mozilla/Add-ons/WebExtensions/manifest.json/developer#browser_compatibility) 关键字，以获取扩展的开发者和作者的详细信息。
- **扩展功能。** 例如，在编写本文时，Edge 不支持扩展定义快捷键的 [`commands`](https://developer.mozilla.org/en-US/docs/Mozilla/Add-ons/WebExtensions/manifest.json/commands#Browser_compatibility) 字段。
- **字段可选性。** 例如，在编写本文时，在 Edge 中 [`author`](https://developer.mozilla.org/zh-CN/docs/Mozilla/Add-ons/WebExtensions/manifest.json/author#browser_compatibility) 字段是必需的，但在其他主要浏览器中是可选的。

浏览器兼容性信息包含在 Mozilla Developer Network [`manifest.json`](https://developer.mozilla.org/zh-CN/docs/Mozilla/Add-ons/WebExtensions/manifest.json) 页的每个字段中。
...
|         | 注册费 | 上传模块    | 发布审核               | 开发者账号需要 2FA 验证 |
| ------- | --- | ------- | ------------------ | -------------- |
| Firefox | 否   | web-ext | 全自动，仅需要几秒钟 [^1]    | 否              |
| Chrome  | 是   | 是       | 全自动，短于一小时          | 是              |
| Opera   | 否   | 否       | 人工审核，但不需要提供 SLA    | 否              |
| Edge    | 是   | 否       | 人工审核，需要 72 小时 [^2] | 是              |
