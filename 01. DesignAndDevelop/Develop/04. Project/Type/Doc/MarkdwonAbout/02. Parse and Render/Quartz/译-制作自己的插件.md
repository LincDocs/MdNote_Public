...
### 1.4. externalResource

- `textTransform`在将文件解析为 [Markdown AST](https://github.com/syntax-tree/mdast) _之前_，执行文本到文本的转换。
- `markdownPlugins`[定义了remark 插件](https://github.com/remarkjs/remark/blob/main/doc/plugins.md)的列表。`remark`是一个将 Markdown 转换为结构化 Markdown 的工具。
- `htmlPlugins`[定义了rehype 插件](https://github.com/rehypejs/rehype/blob/main/doc/plugins.md)列表。与 的工作方式类似`remark`，`rehype`它是一个以结构化方式将 HTML 转换为 HTML 的工具。
...
`visit`最后，您还可以使用包中的函数`unist-util-visit`或包`findAndReplace`中的函数对 Markdown 或 HTML AST 进行转换`mdast-util-find-and-replace`。
