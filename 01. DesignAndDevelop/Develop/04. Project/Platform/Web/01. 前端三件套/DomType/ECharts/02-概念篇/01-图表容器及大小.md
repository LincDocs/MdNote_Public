...
在[快速上手](https://echarts.apache.org/handbook/zh/get-started)中，我们介绍了初始化 ECharts 的接口 [`echarts.init`](https://echarts.apache.org//api.html#echarts.init)。[API 文档](https://echarts.apache.org//api.html#echarts.init)中详细介绍了参数的具体含义，建议理解后再阅读本文。
...
需要注意的是，使用这种方法在调用 `echarts.init` 时*需保证容器已经有宽度和高度*了。
