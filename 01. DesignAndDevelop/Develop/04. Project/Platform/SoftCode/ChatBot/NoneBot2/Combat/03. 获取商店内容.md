...
NoneBot 提供了一个[商店](https://nonebot.dev/store/plugins)，商店内容均由社区开发者贡献。你可以在商店中查找你需要的适配器和插件等，进行安装或者参考其文档等。
...
请注意 `nb-cli` 并不会在安装驱动器后修改项目所使用的驱动器，请自行参考[配置方法](https://nonebot.dev/docs/appendices/config)章节以及 [`DRIVER` 配置项](https://nonebot.dev/docs/appendices/config#driver)修改驱动器。
