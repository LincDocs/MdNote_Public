...
在之前的示例中，我们已经了解了如何[获取事件信息](../02.%20指南/07.%20获取事件信息.mdx.md)以及[使用平台接口](05.%20使用平台接口.mdx.md)。但是，事件信息通常不仅仅包含消息这一个内容，还有其他平台提供的信息，例如消息发送时间、消息发送者等等。同时，在使用平台接口时，我们需要确保使用的**平台接口**与所要发送的**平台类型**一致，对不同类型的事件需要做出不同的处理。在本章节中，我们将介绍如何获取事件更多的信息以及根据事件类型进行不同的处理。

## 事件类型

在 NoneBot 中，事件均是 `nonebot.adapters.Event` 基类的子类型，基类对一些必要的属性进行了抽象，子类型则根据不同的平台进行了实现。在[自定义权限](06.%20权限控制.mdx.md#自定义权限)一节中，我们就使用了 `Event` 的抽象方法 `get_user_id` 来获取事件发送者 ID，这个方法由协议适配器进行了实现，返回机器人用户对应的平台 ID。更多的基类抽象方法可以在[使用适配器](../04.%20进阶/02.%20使用适配器.md#获取事件通用信息)中查看。
...
:::tip 提示
如何更好地编写一个跨平台的插件，我们将在[最佳实践](../05.%20最佳实践/05.%20插件跨平台支持.mdx.md)中介绍。
:::
