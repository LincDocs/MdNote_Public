...
在[指南](../02.%20指南/05.%20事件响应器.md)与[深入](../03.%20深入/02.%20响应规则.md)中，我们已经介绍了事件响应器的基本用法以及响应规则、权限控制等功能。在这一节中，我们将介绍事件响应器的组成，内置的响应规则，与第三方响应规则拓展。
...
事件触发权限 `permission` 是一个 `Permission` 对象，这在[权限控制](../03.%20深入/06.%20权限控制.mdx.md)一节中已经介绍过。事件触发权限会在事件响应器的类型检查通过后进行检查，如果权限检查通过，则执行响应规则检查。

### 事件响应规则

事件响应规则 `rule` 是一个 `Rule` 对象，这在[响应规则](../03.%20深入/02.%20响应规则.md)一节中已经介绍过。事件响应器的响应规则会在事件响应器的权限检查通过后进行匹配，如果响应规则检查通过，则触发该响应器。
...
在部分情况中，可以使用 [`stop_propagation`](../03.%20深入/03.%20会话控制.mdx.md#stop_propagation) 方法动态阻止事件传播，该方法需要 handler 在参数中获取 matcher 实例后调用方法。
...
:::tip
响应规则的使用方法可以参考 [深入 - 响应规则](../03.%20深入/02.%20响应规则.md)。
:::
...
`command` 是最常用的响应规则，它用于匹配消息是否为命令。它会根据配置中的 [Command Start 和 Command Separator](../03.%20深入/01.%20配置.mdx.md#command-start-和-command-separator) 来判断消息是否为命令。
...
命令解析后的结果可以通过 [`Command`](07.%20依赖注入.mdx.md#command)、[`RawCommand`](07.%20依赖注入.mdx.md#rawcommand)、[`CommandArg`](07.%20依赖注入.mdx.md#commandarg)、[`CommandStart`](07.%20依赖注入.mdx.md#commandstart)、[`CommandWhitespace`](07.%20依赖注入.mdx.md#commandwhitespace) 依赖注入获取。
...
参数解析后的结果可以通过 [`ShellCommandArgv`](07.%20依赖注入.mdx.md#shellcommandargv)、[`ShellCommandArgs`](07.%20依赖注入.mdx.md#shellcommandargs) 依赖注入获取。
...
正则匹配后的结果可以通过 [`RegexStr`](07.%20依赖注入.mdx.md#regexstr)、[`RegexGroup`](07.%20依赖注入.mdx.md#regexgroup)、[`RegexDict`](07.%20依赖注入.mdx.md#regexdict) 依赖注入获取。
...
详情请阅读最佳实践中的 [命令解析拓展](../05.%20最佳实践/07.%20Alconna命令解析拓展/02.%20Alconna%20插件.mdx.md) 章节。
