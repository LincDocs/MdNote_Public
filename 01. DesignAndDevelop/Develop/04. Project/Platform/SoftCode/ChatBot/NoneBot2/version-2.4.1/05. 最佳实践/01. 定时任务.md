...
在使用前请先安装 `nonebot-plugin-apscheduler` 插件至项目环境中，可参考[获取商店插件](../02.%20指南/03.%20获取商店内容.mdx.md#安装插件)来了解并选择安装插件的方式。如：
...
由于 `nonebot_plugin_apscheduler` 作为插件，因此需要在使用前对其进行**加载**并**导入**其中的 `scheduler` 调度器来创建定时任务。使用 `require` 方法可轻松完成这一过程，可参考 [跨插件访问](../04.%20进阶/05.%20跨插件访问.md) 一节进行了解。
...
:::caution 注意
由于 APScheduler 的定时任务并不是**由事件响应器所触发的事件**，因此其任务函数无法同[事件处理函数](../02.%20指南/06.%20事件处理.mdx.md#事件处理函数)一样通过[依赖注入](../02.%20指南/07.%20获取事件信息.mdx.md#认识依赖注入)获取上下文信息，也无法通过事件响应器对象的方法进行任何操作，因此我们需要使用[调用平台 API](../03.%20深入/05.%20使用平台接口.mdx.md#调用平台-api)的方式来获取信息或收发消息。
