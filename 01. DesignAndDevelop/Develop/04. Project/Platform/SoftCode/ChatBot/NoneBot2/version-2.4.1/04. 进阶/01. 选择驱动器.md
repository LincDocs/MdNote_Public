...
:::tip 提示
如何**安装**驱动器请参考[安装驱动器](../02.%20指南/store.mdx#安装驱动器)。
:::
...
驱动器的配置方法已经在[配置](../03.%20深入/config.mdx)章节中简单进行了介绍，这里将详细介绍驱动器配置的格式。
...
特别的，为了简化内置驱动器模块路径，我们可以使用 `~` 符号作为内置驱动器模块路径的前缀，如 `~fastapi` 代表使用内置驱动器 `fastapi`。NoneBot 内置了多个驱动器适配，但需要安装额外依赖才能使用，具体请参考[安装驱动器](../02.%20指南/store.mdx#安装驱动器)。常见的驱动器配置如下：
