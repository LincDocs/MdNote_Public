...
:::tip 提示
如何**安装**驱动器请参考[安装驱动器](../02.%20指南/03.%20获取商店内容.mdx.md#安装驱动器)。
:::
...
1. ASGI 应用框架，具有以下功能：
   ...
   - 用户可以向 ASGI 应用添加任何服务端相关功能，如：[添加自定义路由](10.%20添加路由.md)。

## 配置驱动器

驱动器的配置方法已经在[配置](../03.%20深入/01.%20配置.mdx.md)章节中简单进行了介绍，这里将详细介绍驱动器配置的格式。
...
特别的，为了简化内置驱动器模块路径，我们可以使用 `~` 符号作为内置驱动器模块路径的前缀，如 `~fastapi` 代表使用内置驱动器 `fastapi`。NoneBot 内置了多个驱动器适配，但需要安装额外依赖才能使用，具体请参考[安装驱动器](../02.%20指南/03.%20获取商店内容.mdx.md#安装驱动器)。常见的驱动器配置如下：
