...
在[驱动器](01.%20选择驱动器.md)一节中，我们了解了驱动器的两种类型。既然驱动器可以作为服务端运行，那么我们就可以向驱动器添加路由规则，从而实现自定义的 API 接口等功能。在添加路由规则时，我们需要注意驱动器的类型，详情可以参考[选择驱动器](01.%20选择驱动器.md#配置驱动器)。
...
对于 `Request` 和 `Response` 的详细信息，可以参考 [API 文档](../07.%20api/drivers/index.md)。
...
对于 `WebSocket` 的详细信息，可以参考 [API 文档](../07.%20api/drivers/index.md)。
...
在获取到了 ASGI 应用后，我们就可以直接使用 ASGI 应用框架提供的功能来添加路由规则了。这里我们以 [FastAPI](01.%20选择驱动器.md#fastapi默认) 为例，演示如何添加路由规则。
