...
use std::{
    fs,
    io::{prelude::*, BufReader}, // [!code ++]
    net::{TcpListener, TcpStream}, // [!code]
    thread,
    time::Duration,
};
// --snip--

fn handle_connection(mut stream: TcpStream) { // [!code ++]
    // --snip--
...
下面来试试，启动服务器后，打开你的浏览器，这次要分别打开两个页面(tab页): `http://127.0.0.1:7878/` 和 `http://127.0.0.1:7878/sleep`。
