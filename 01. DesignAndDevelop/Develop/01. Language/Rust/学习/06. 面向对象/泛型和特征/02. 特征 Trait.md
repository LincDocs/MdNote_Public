...
如果我们想定义一个文件系统，那么把该系统跟底层存储解耦是很重要的。

- 文件操作主要包含四个：`open` 、`write`、`read`、`close`
- 这些操作可以发生在硬盘，可以发生在内存，还可以发生在网络 IO 甚至（...我实在编不下去了，大家来帮帮我）
...
// 含义:
// 它在我们定义的类型(`struct`)上自动派生 `Debug` 特征，
// 接着可以使用 `println!("{:?}", x)` 打印这个类型
```
...
// 含义:
// 通过 `std::ops::Add` 特征来限制 `T`，
// 只有 `T` 实现了 `std::ops::Add` 才能进行合法的加法操作，
// 毕竟不是所有的类型都能进行相加。
```
...
特征*只定义行为看起来是什么样的，而不定义行为具体是怎么样的* (类似于纯抽象基类)。因此，我们只定义特征方法的签名，而不进行实现，此时方法签名结尾是 `;`，而不是一个 `{}`。
...
impl Summary for Post { // 为文章实现 '总结' 特征
    fn summarize(&self) -> String {
...
impl Summary for Weibo { // 为微博实现 '总结' 特征
    fn summarize(&self) -> String {
...
> 什么是孤儿规则：
> 
> 定义: 关于特征实现与定义的位置，有一条非常重要的原则：*如果你想要为类型 `A` 实现特征 `T`，那么 `A` 或者 `T` **至少有一个**是在当前作用域中定义的！*
> 
> 好处: 该规则被称为**孤儿规则**，可以确保其它人编写的代码不会破坏你的代码，也确保了你不会莫名其妙就破坏了风马牛不相及的代码。
> 
> - 例子1: 类型在当前作用域：
>   例如我们可以为上面的 `Post` 类型实现标准库中的 `Display` 特征。这是因为 `Post` 类型定义在当前的作用域中
> - 例子2: 特征在当前作用域：
>   我们也可以在当前包中为 `String` 类型实现 `Summary` 特征，因为 `Summary` 定义在当前作用域中
> - 例子3: 均不满足
...
> 与其他语言对比
> 
> 这里就相当于多态的特性，里氏替换原则。在其他面向对象语言中，这里就相当于使用基类参数来接受任意派生类参数

### 非语法糖写法、特征约束 (后未)

虽然 `impl Trait` 这种语法非常好理解，但是实际上它只是一个**语法糖**：

真正的完整书写形式如下所述，形如 `T: Summary` 被称为**特征约束**。（前面那种不算吗？）
...
例如一个函数接受两个 `impl Summary` 的参数：（相当于接受多个基类参数类型）
...
以下两个例子较为复杂，目的是为读者展示下真实的使用场景长什么样，因此需要读者细细阅读，最终消化这些知识对于你的 Rust 之路会有莫大的帮助。
...
在Rust语言中，特征(traits)确实主要用于定义方法(methods)接口，而不能直接定义具有实际存储的成员变量(fields)。这是Rust特征与一些其他语言中类似概念(如Java中的接口)的一个重要区别。
