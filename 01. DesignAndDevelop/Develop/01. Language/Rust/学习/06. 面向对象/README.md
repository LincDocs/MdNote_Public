...
我们这里先不详细展开讲解，只是先建立对方法定义的大致印象。下面的图片将 **Rust 方法定义与其它语言的方法定义**做了对比：

![](https://pica.zhimg.com/80/v2-0d848e960f3279999eab4b1317f6538e_1440w.png)

可以看出，其它语言中所有定义都在 `class` 中，但是 Rust 的**对象定义和方法定义是分离**的，这种数据和使用分离的方式，会给予使用者极高的灵活度。
...
> 与其他语言不同
> 
> 其他语言的构造函数/工厂函数/静态函数，在Rust中与 “关联函数” 有关，这更类似于静态工厂函数的定位。也并不局限只有一个
> 
> 名字上不同于Cpp的同名表示构造函数、js的 `constructor` 或 python `__init__(self)` 表示构造函数。rust不限制名字，但约定使用 `new` 作为构造器 (是的，Rust没有用 `new` 作为关键字)
